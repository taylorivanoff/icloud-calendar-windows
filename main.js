...
async function saveSharedCookies(ses) {
  try {
    // Get cookies from both icloud.com and apple.com domains
  ...
  } catch (e) { console.error('Error saving cookies:', e); }
}
...
async function createWindow() {
  ...
  mainWindow.loadURL('https://www.icloud.com/calendar');
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip('iCloud Calendar');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Show iCloud Calendar', click: () => { if (!mainWindow) createWindow(); mainWindow.show(); } },
